...
class TestSourceDataLoadService(BaseTest):
    ...
    @pytest.mark.asyncio
    async def test_load_data_from_canvas_should_not_create_existing_attendances(
        self,
        source_data_load_service,
        create_user,
        create_canvas_user,
        ...
        with attendance_repo.session():
            ...
            for att in atts:
                assert att.status == AttendanceStatus.COMPLETED
                assert att.value is not None
            attendance = attendance_repo.get_by_db_id(att.id)
            assert attendance.value == AttendanceValue.EXCUSE
