    ...
    def __init__(
        ...
        *,
        ...
        _message = (
            err.message
            if err is not None
            else message if message is not None else self.SERVER_ERROR_MESSAGE
        )
        _code = (
            err.code
            if err is not None
            else code if code is not None else CoreErrors.UNKNOWN.value
        )
        data = err.data if err is not None and err.data is not None else None
        response_payload: dict[str, str | dict[str, str]] = {
            "message": _message,
            "code": _code,
        }
        if data is not None:
